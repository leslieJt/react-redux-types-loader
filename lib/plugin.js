  ...
  return {
    ...
    visitor: {
      ExportNamedDeclaration(path, state) {
        const module_path = state.opts.module_path;
        ...
        declaration.get('declarations').map((item, index) => {
          if (item.isVariableDeclarator()) {
            ...
            let value = item.get('init').node ? item.get('init').node.value : '';
            let str = (module_path ? module_path : +new Date()) + '/' + (value ? value.toString() : item.get('id').node.name);
            let cwd = process.cwd();
